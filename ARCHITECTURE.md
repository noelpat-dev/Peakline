...
- Warm route stores are generation-aware. Coach publishes readiness, training call, recommended split, and Preview handoff together; Sleep caches are keyed by exact bounded signatures; Progress publishes summaries plus value-only exercise rows; Nutrition publishes dashboard and Insights payloads. Older async results are rejected.
