...
- Coach routes render from a complete `CoachRouteRenderSnapshot`, including intelligence, daily decision, the recommended value-only `WorkoutPreviewSplit`, derived metrics, sleep analytics, practical weekly summary, and `WeeklyReview`. The first frame contains the real actionable Today's Call hero plus its prepared “Why this?” context and installs no SwiftData query; deeper supporting sections mount about 50 ms later from the same snapshot, then bounded live queries and observation attach. Provisional decisions retain the split name as the headline and use a dedicated Provisional badge. No empty or intermediate warm shell is mounted, the primary Preview action is usable immediately, and scene-phase changes never replace mounted Coach content with a placeholder.
