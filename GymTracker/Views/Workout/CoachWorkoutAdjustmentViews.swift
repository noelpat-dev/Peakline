...
struct AdaptiveWorkoutActionsCard: View {
    @Environment(\.appTheme) private var appTheme

    let guidance: AdaptiveWorkoutGuidance
    let recommendations: [CoachWorkoutActionRecommendation]
    let appliedAdjustment: AppliedCoachWorkoutAdjustment?
    let selectAction: (CoachWorkoutAdjustmentAction) -> Void
    let reset: () -> Void
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Preview changes first. Nothing edits your split template.")
                            .font(.subheadline)
                ...
                if let appliedAdjustment {
                    appliedBanner(appliedAdjustment)
                }

                VStack(spacing: 10) {
                    ForEach(recommendations) { recommendation in
                        Button {
                            selectAction(recommendation.action)
                        } label: {
    ...
    private func appliedBanner(_ adjustment: AppliedCoachWorkoutAdjustment) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("\(adjustment.title) applied")
                        .font(.subheadline.weight(.semibold))
                        .foregroundStyle(appTheme.colors.textPrimary)

                    Text("\(adjustment.preview.beforeTotalSets) to \(adjustment.preview.afterTotalSets) sets in this preview.")
                        .font(.caption)
            ...
            Button {
                ...
                Label("Reset to original plan", systemImage: "arrow.counterclockwise")
                    .font(.subheadline.weight(.semibold))
            }
        ...
        .padding(12)
        .background(appTheme.colors.success.opacity(0.12), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
    }

    private func actionRow(_ recommendation: CoachWorkoutActionRecommendation) -> some View {
        HStack(alignment: .top, spacing: 12) {
            FitnessIconBadge(
                systemImage: recommendation.action.systemImage,
                size: 38,
                tint: recommendation.isCoachSuggested ? appTheme.colors.accent : appTheme.colors.textSecondary,
                background: recommendation.isCoachSuggested ? appTheme.colors.accentSurface : appTheme.elevatedCardBackground
            )
        ...
        .padding(12)
        .background(appTheme.elevatedCardBackground, in: RoundedRectangle(cornerRadius: 12, style: .continuous))
        .overlay {
...
struct CoachActionHistoryList: View {
    @Environment(\.appTheme) private var appTheme
        ...
        if entries.isEmpty {
                ...
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("No coach actions yet")
                            .font(.headline)
                            .foregroundStyle(appTheme.colors.textPrimary)
                        Text("Applied, cancelled, reset, and bypassed workout recommendations will appear here.")
                            .font(.subheadline)
                            .foregroundStyle(appTheme.colors.textSecondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
            ...
            LazyVStack(spacing: 12) {
                ForEach(entries.prefix(5)) { entry in
                    CoachActionHistoryRow(entry: entry)
                }
            }
        }
    }
...
private struct CoachActionHistoryRow: View {
    @Environment(\.appTheme) private var appTheme
        ...
        FitnessCard(style: .compact) {
            DisclosureGroup {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    detailLine("Fatigue risk", entry.fatigueRiskLevel.displayName)
                    detailLine("Confidence", entry.confidence.displayName)
                    detailLine("Sets", "\(entry.beforeTotalSets) to \(entry.afterTotalSets)")
                    if let splitName = entry.splitName {
                        detailLine("Workout", splitName)
                    }
