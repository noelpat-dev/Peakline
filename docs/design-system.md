...
Use native system materials and existing SDK availability checks. Use the iPhone 17 simulator for repeatable validation and a supported physical device for hardware-only behaviour.
...
Implementation and latency ownership live in [Architecture](architecture.md) and [Performance](performance.md). This guide owns what the interaction must feel like.
...
- Native sheets present their usable content immediately; do not add a second delayed inner reveal. Dismissal and primary actions remain available from the first frame.
...
- **Target devices:** use the iPhone 17 simulator for repeatable smoke and performance checks, then a supported physical iPhone for haptics, high-refresh-rate motion, camera/HealthKit access, and final clipped-row or crowded-control checks.
...
- **Live Logger:** prioritise timer, current exercise, weight/reps controls, rest timer, and finish. Keep set rows free of a separate Complete Set button; entered sets are completed when the workout is finished. While a rest timer is active, show its countdown, +30s, and Skip controls once; keep the manual duration presets only in the dedicated Rest Timer section. Show Warm-up or Logged when meaningful; do not label an ordinary incomplete set Draft. Continue advances immediately and uses the session's nonrepeating motivational rotation as brief nonblocking inline feedback. The rating popup uses a single scrim and a fully opaque semantic card with full-strength text and enabled rating controls on its first visible frame. Before rating a session recorded at four hours or more, present the measured duration with Edit Duration, Use Recorded Time, and Cancel paths. Final completion follows the PR-only celebration rule.
- **Session Summary:** lead with completed work, duration, rating, genuine improvements, and one practical next suggestion. Wrap long split titles at their semantic font size; stack the header and supporting metrics at accessibility text sizes.
...
- **Nutrition and Hydration:** use compact reviewable summaries, honest permission or unavailable states, and clear editing boundaries. Nutrition uses a flat date-navigation row matching History: a bold centred day label and symmetric previous/next controls, without an enclosing card or date pill. Tapping the day opens a native calendar popover. The prepared date row and content share one persistent dashboard. Past-day intake headings identify the selected day. Keep meal groups as cards with flat internal food rows and dividers, retaining swipe deletion. Nutrition action, target, comparison and import grids, plus Workout mode choices, use one column at accessibility text sizes. Preserve optional fibre progress and visibly read-only past days.
